...
import { wrapEmailHtml, emailButton, emailButtonOutline, emailButtonsRow, emailStatusBadge, emailInfoCard, emailHighlightCard, emailDueChange, emailTicketHeader, emailGreeting, emailSignatureBlock, shortTicketId, enhanceForExecutor, enhanceForClient, } from '../helpers/emailTemplates.js';
...
const buildEmailFromTemplate = async (ctx) => {
    ...
    const buildAiDescriptionBlock = async (rawDescription, label) => {
        if (!rawDescription)
            return '';
        ...
        return emailInfoCard(`<strong>${escapeHtml(blockLabel)}</strong><br/><div style="white-space:pre-wrap;margin-top:8px;">${escapeHtml(enhanced)}</div>`);
    };
    switch (ctx.eventType) {
        case 'created': {
            ...
            const bodyHtml = emailGreeting(ctx.recipientName) +
                emailTicketHeader(sid, ctx.ticketTitle) +
                emailInfoCard(`<strong>Статус:</strong> Зарегистрирована<br/>` +
        ...
        case 'accepted': {
            ...
            const bodyHtml = emailGreeting(ctx.recipientName) +
                ...
                emailInfoCard(`<strong>Плановый срок:</strong> ${escapeHtml(ctx.dueDatetime)}`) +
                aiBlock +
                emailButtonsRow([openBtn]) +
                signatureHtml;
            const html = wrapEmailHtml(bodyHtml);
        ...
        case 'need_info': {
            ...
            const bodyHtml = emailGreeting(ctx.recipientName) +
                ...
                emailHighlightCard(`<strong>Вопросы:</strong><br/><div style="white-space:pre-wrap;margin-top:8px;">${escapeHtml(ctx.questionsList)}</div>`) +
        ...
        case 'info_received': {
            ...
            const bodyHtml = emailGreeting(ctx.recipientName) +
                ...
                emailInfoCard(`Спасибо! Продолжаем работу.<br/><strong>Плановый срок:</strong> ${escapeHtml(ctx.dueDatetime)}`) +
                emailButtonsRow([openBtn]) +
                signatureHtml;
            const html = wrapEmailHtml(bodyHtml);
        ...
        case 'rejected': {
            ...
            const bodyHtml = emailGreeting(ctx.recipientName) +
                ...
                emailHighlightCard(`<strong>Причина:</strong> ${escapeHtml(ctx.commentText)}`) +
                emailInfoCard(`<strong>Возможные варианты:</strong><br/><div style="white-space:pre-wrap;margin-top:8px;">${escapeHtml(ctx.rejectionOptions)}</div>`) +
        ...
        case 'waiting_reminder': {
            ...
            const bodyHtml = emailGreeting(ctx.recipientName) +
                ...
                emailHighlightCard(`<strong>Нужна информация:</strong><br/><div style="white-space:pre-wrap;margin-top:8px;">${escapeHtml(ctx.questionsList)}</div>`) +
...
const ensureStorageReady = async () => {
    ...
    storageInitPromise = (async () => {
        await dbQuery(`create table if not exists app_notification_dedupe (
            dedupe_key text primary key,
            last_seen_at timestamptz not null default now()
        )`);
        await dbQuery(`create index if not exists idx_app_notification_dedupe_last_seen_at
            on app_notification_dedupe(last_seen_at)`);
        await dbQuery(`create table if not exists app_notification_digest_queue (
        ...
        await dbQuery(`create index if not exists idx_app_notification_digest_queue_pending
            on app_notification_digest_queue(sent_at, created_at)`);
        await dbQuery(`create table if not exists app_notification_event_log (
            id bigserial primary key,
        ...
        await dbQuery(`create index if not exists idx_app_notification_event_log_project_created
            on app_notification_event_log(project_id, created_at desc)`);
        await dbQuery(`create index if not exists idx_app_notification_event_log_recipient_created
            on app_notification_event_log(recipient_email, created_at desc)`);
        await dbQuery(`create table if not exists app_notification_runtime_state (
        ...
        logger.error('Failed to initialize notification storage', {
...
const flushDigestState = async (slotKey, nowDate) => {
    ...
    for (const recipient of grouped.values()) {
        ...
        const payload = {
            ...
            dedupeKey: `ticket_client:digest:${slotKey}:${recipient.recipientEmail}`,
...
export const startTicketClientDigestScheduler = () => {
    ...
    void ensureStorageReady().catch((error) => {
        logger.error('Ticket digest scheduler storage init failed', {
            error: serializeError(error),
        });
    });
