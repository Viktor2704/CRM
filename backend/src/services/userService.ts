...
import {
    CLIENT_ROLES,
    CONTRACTOR_ROLES,
    INTERNAL_ASSIGNED_ROLES,
    roleAllowsGlobalBinding,
    roleRequiresContractorCounterparty,
    roleRequiresCounterparty,
    roleRequiresTenantCounterparty,
} from '../types.js';
const clientRoles = Array.from(CLIENT_ROLES);
const externalRoles = [...Array.from(CLIENT_ROLES), ...Array.from(CONTRACTOR_ROLES)];
...
const loadCounterpartyType = async (client, counterpartyId) => {
    const result = await client.query(`select coalesce(type, 'tenant') as type
       from tenants
       where id = $1::uuid
...
const assertRoleBindingsConsistency = async (client, role, bindings) => {
    const normalizedRole = String(role ?? '').trim();
    const counterpartyId = typeof bindings?.counterpartyId === 'string' ? bindings.counterpartyId.trim() : '';
    if (roleAllowsGlobalBinding(normalizedRole)) {
        if (counterpartyId) {
            throw new ApiError(422, 'GLOBAL_ROLE_COUNTERPARTY_FORBIDDEN', 'Global internal role cannot have counterparty');
        }
        if (!bindings.isGlobal) {
            throw new ApiError(422, 'GLOBAL_ROLE_SCOPE_INVALID', 'Global internal role must have global access');
        }
        return;
    }
    if (INTERNAL_ASSIGNED_ROLES.has(normalizedRole)) {
        if (counterpartyId) {
            throw new ApiError(422, 'ASSIGNED_ROLE_COUNTERPARTY_FORBIDDEN', 'Assigned internal role cannot have counterparty');
        }
        if (bindings.isGlobal) {
            throw new ApiError(422, 'ASSIGNED_ROLE_GLOBAL_FORBIDDEN', 'Assigned internal role cannot have global access');
        }
        return;
    }
    if (!roleRequiresCounterparty(normalizedRole)) {
        throw new ApiError(422, 'USER_ROLE_INVALID', 'Unsupported user role');
    }
    if (!counterpartyId) {
        throw new ApiError(422, 'COUNTERPARTY_ID_REQUIRED', 'Counterparty is required for this role');
    }
    if (!isUuidValue(counterpartyId)) {
        throw new ApiError(422, 'COUNTERPARTY_ID_INVALID', 'Counterparty must be a valid UUID');
    }
    if (bindings.isGlobal) {
        throw new ApiError(422, 'EXTERNAL_ROLE_GLOBAL_FORBIDDEN', 'External role cannot have global access');
    }
    const counterpartyType = await loadCounterpartyType(client, counterpartyId);
    if (!counterpartyType) {
        throw new ApiError(422, 'COUNTERPARTY_NOT_FOUND', 'Counterparty not found');
    }
    if (roleRequiresTenantCounterparty(normalizedRole) && counterpartyType !== 'tenant') {
        throw new ApiError(422, 'TENANT_ROLE_REQUIRES_TENANT', 'Client role must be bound to tenant counterparty');
    }
    if (roleRequiresContractorCounterparty(normalizedRole) && counterpartyType !== 'contractor') {
        throw new ApiError(422, 'CONTRACTOR_ROLE_REQUIRES_CONTRACTOR', 'Contractor role must be bound to contractor counterparty');
    }
};
...
export const createUser = async (actorUserId, input, meta) => {
    ...
    try {
        return await withTx(async (client) => {
            await assertRoleBindingsConsistency(client, input.role, bindings);
    ...
    try {
        return await withTx(async (client) => {
          ...
          group by u.id, b.counterparty_id, b.is_global`, [userId]);
            ...
            await assertRoleBindingsConsistency(client, nextRole, nextBindings);
...
export const listUsers = async (query) => {
    ...
    if (query.tab === 'employees') {
        where.push(`u.role::text <> all(${push(externalRoles)}::text[])`);
        where.push(`u.status::text <> 'deactivated'`);
    }
    else if (query.tab === 'clients') {
        where.push(`u.role::text = any(${push(externalRoles)}::text[])`);
        where.push(`u.status::text <> 'deactivated'`);
    }
    ...
    const [totalResult, listResult] = await Promise.all([
        dbQuery(`select count(*)::text as total from app_users u ${whereSql}`, values),
        dbQuery(`${userSelectSql}
      ${whereSql}
