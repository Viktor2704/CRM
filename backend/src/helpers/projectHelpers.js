...
import { projectAdminRoles, projectClientRoles, projectOpsRoles } from './roles.js';
...
export const appendProjectScopeCondition = async (params) => {
    ...
    const scopedQuery = typeof queryFn === 'function' ? queryFn : dbQuery;
    const bindingResult = await scopedQuery(`select counterparty_id::text as counterparty_id
       from app_user_bindings
       where user_id = $1::uuid
       limit 1`, [actorUserId]);
    const binding = bindingResult.rows[0] ?? {};
    const counterpartyId = normalizeText(binding.counterparty_id);
    if (projectAdminRoles.has(actorRole)) {
        return;
    }
    if (projectClientRoles.has(actorRole)) {
        ...
        conditions.push(`r.tenant_id = $${values.length}::text`);
        return;
    }
    if (projectOpsRoles.has(actorRole)) {
        ...
        conditions.push(`(
          exists (
            ...
            where upb.user_id = ${actorRef}::uuid
              and upb.project_id = r.id
          )
          or exists (
            select 1
            from app_user_direction_bindings udb
            where udb.user_id = ${actorRef}::uuid
              and udb.direction_id::text = r.direction_id
          )
          or r.created_by_id = ${actorRef}::text
