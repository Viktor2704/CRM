...
export const enhanceDescriptionWithAI = async (description, subject) => {
    if (!description)
        return description;
    try {
        const result = await callLLM(AI_EMAIL_SYSTEM_PROMPT, `Тема заявки: ${subject}\n\nОписание работ:\n${description}`, { timeoutMs: 5000, maxTokens: 512, temperature: 0.3 });
...
3. Норма��ивное основание — укажи применимые документы понятным языком:
...
export const enhanceForExecutor = async (description, subject) => {
    if (!description)
        return description;
    try {
        const result = await callLLM(AI_EXECUTOR_INSTRUCTION_PROMPT, `Тема заявки: ${subject}\n\nОписание работ:\n${description}`, { timeoutMs: 8000, maxTokens: 768, temperature: 0.3 });
        return result || description;
    }
    catch {
        return description;
...
export const enhanceForClient = async (description, subject) => {
    if (!description)
        return description;
    try {
        const result = await callLLM(AI_CLIENT_DESCRIPTION_PROMPT, `Тема заявки: ${subject}\n\nОписание работ:\n${description}`, { timeoutMs: 6000, maxTokens: 512, temperature: 0.3 });
        return result || description;
    }
    catch {
        return description;
...
export const wrapEmailHtml = (bodyHtml, options) => {
...
<body style="margin:0;padding:0;background:#EAECF0;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;">
  ${preheaderHtml}
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#EAECF0;padding:32px 0;">
    <tr>
      <td align="center">
        <table role="presentation" class="email-container" width="640" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;background:#FFFFFF;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
          <!-- Header -->
          <tr>
            <td class="email-header" style="background:#B91C1C;padding:28px 36px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="vertical-align:middle;">
                    <div style="color:#FFFFFF;font-family:Arial,Helvetica,sans-serif;font-size:24px;font-weight:800;letter-spacing:1px;line-height:1.2;">НОВИНЖСТРОЙ</div>
                    <div style="color:rgba(255,255,255,0.75);font-family:Arial,Helvetica,sans-serif;font-size:12px;letter-spacing:0.5px;margin-top:4px;">Системы безопасности</div>
                  </td>
                  <td align="right" style="vertical-align:middle;">
                    <div style="width:42px;height:42px;background:rgba(255,255,255,0.15);border-radius:50%;text-align:center;line-height:42px;display:inline-block;">
                      <span style="color:#FFFFFF;font-size:18px;font-weight:700;">N</span>
                    </div>
                  </td>
                </tr>
              </table>
          ...
          <!-- Body -->
          <tr>
            <td class="email-body" style="padding:36px 40px;font-family:Arial,Helvetica,sans-serif;color:#1E293B;font-size:15px;line-height:1.7;">
              ${bodyHtml}
            </td>
          </tr>
          <!-- Divider -->
          ...
          <!-- Footer -->
          <tr>
            <td class="email-footer" style="padding:24px 40px 28px;background:#F8FAFC;">
              <table role="presentation" class="email-footer-inner" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="color:#64748B;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.6;">
                    <strong style="color:#475569;">Новинжстрой</strong> &mdash; управление обслуживанием<br/>
                  ...
                  <td align="right" style="vertical-align:bottom;color:#94A3B8;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1.5;">
                    &copy; Новинжстрой, 2026<br/>
                    <a href="https://novinzhstroy.ru" style="color:#94A3B8;text-decoration:none;">novinzhstroy.ru</a>
                  </td>
                </tr>
              </table>
              <p style="margin:12px 0 0;color:#94A3B8;font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:1.5;">Это автоматическое уведомление. Если вы получили его по ошибке — просто проигнорируйте.</p>
            </td>
          </tr>
        </table>
      </td>
...
export const emailButton = (href, label, color = '#DC2626') => `<a href="${escapeHtml(href)}" style="display:inline-block;padding:16px 40px;background:${color};color:#FFFFFF;text-decoration:none;border-radius:8px;font-weight:700;font-family:Arial,Helvetica,sans-serif;font-size:16px;margin:4px;">${escapeHtml(label)}</a>`;
export const emailButtonOutline = (href, label, color = '#DC2626') => `<a href="${escapeHtml(href)}" style="display:inline-block;padding:14px 40px;background:#FFFFFF;color:${color};border:2px solid ${color};text-decoration:none;border-radius:8px;font-weight:700;font-family:Arial,Helvetica,sans-serif;font-size:16px;margin:4px;">${escapeHtml(label)}</a>`;
export const emailButtonsRow = (buttons) => `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:28px 0;">${buttons.join('&nbsp; ')}</td></tr></table>`;
export const emailStatusBadge = (label, bgColor = '#16a34a', textColor = '#FFFFFF', icon = '') => `<span style="display:inline-block;padding:6px 16px;background:${bgColor};color:${textColor};border-radius:20px;font-weight:700;font-size:13px;font-family:Arial,Helvetica,sans-serif;vertical-align:middle;">${icon ? `${icon} ` : ''}${escapeHtml(label)}</span>`;
export const emailInfoCard = (content, accentColor = '#3B82F6') => `<div style="background:#F8FAFC;border:1px solid #E2E8F0;border-left:4px solid ${accentColor};border-radius:4px 8px 8px 4px;padding:18px 20px;margin:16px 0;font-family:Arial,Helvetica,sans-serif;color:#475569;font-size:14px;line-height:1.7;">${content}</div>`;
export const emailHighlightCard = (content) => `<div style="background:#FEF2F2;border:1px solid #FECACA;border-left:4px solid #DC2626;border-radius:4px 8px 8px 4px;padding:18px 20px;margin:16px 0;font-family:Arial,Helvetica,sans-serif;color:#991B1B;font-size:14px;line-height:1.7;">${content}</div>`;
export const emailSectionTitle = (title) => `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:28px 0 12px;">
  <tr>
    <td style="font-family:Arial,Helvetica,sans-serif;font-size:16px;font-weight:700;color:#1E293B;padding-right:16px;white-space:nowrap;">${escapeHtml(title)}</td>
    <td style="width:100%;"><div style="height:1px;background:#CBD5E1;"></div></td>
  </tr>
</table>`;
export const emailKeyValue = (key, value) => `<tr>
  <td style="padding:6px 16px 6px 0;color:#64748B;font-family:Arial,Helvetica,sans-serif;font-size:14px;white-space:nowrap;vertical-align:top;">${escapeHtml(key)}</td>
  <td style="padding:6px 0;color:#1E293B;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:600;">${escapeHtml(value)}</td>
</tr>`;
export const emailKeyValueTable = (rows) => `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:12px 0 16px;">${rows}</table>`;
export const emailDivider = () => `<div style="height:1px;background:linear-gradient(to right,transparent,#CBD5E1,transparent);margin:24px 0;"></div>`;
export const emailDueChange = (oldDue, newDue) => `<table style="margin:16px 0;font-family:Arial,sans-serif;">
  <tr><td style="padding:6px 16px 6px 0;color:#64748B;font-size:13px;">Было:</td><td style="padding:6px 0;text-decoration:line-through;color:#94A3B8;">${escapeHtml(oldDue)}</td></tr>
  <tr><td style="padding:6px 16px 6px 0;color:#64748B;font-size:13px;">Стало:</td><td style="padding:6px 0;font-weight:700;color:#DC2626;">${escapeHtml(newDue)}</td></tr>
</table>`;
export const emailTicketHeader = (ticketShortId, title) => `<h2 style="margin:0 0 4px;color:#1E293B;font-size:20px;font-weight:700;font-family:Arial,sans-serif;">Заявка №${escapeHtml(ticketShortId)}</h2>
<p style="margin:0 0 20px;color:#64748B;font-size:14px;font-family:Arial,sans-serif;">${escapeHtml(title)}</p>`;
export const emailGreeting = (name) => `<p style="margin:0 0 16px;color:#1E293B;font-size:15px;font-family:Arial,sans-serif;">Здравствуйте, ${escapeHtml(name)}!</p>`;
export const emailSignatureBlock = (signature) => `<div style="margin-top:24px;padding-top:16px;border-top:1px solid #E2E8F0;color:#94A3B8;font-size:13px;font-family:Arial,sans-serif;white-space:pre-line;">${escapeHtml(signature)}</div>`;
export const shortTicketId = (fullId) => fullId && fullId.length > 8 ? fullId.slice(0, 8) : fullId;
export const sendServiceRequestEmail = async (params) => {
  ...
  <td style="padding:10px 12px;border-bottom:1px solid #E2E8F0;color:#475569;">${escapeHtml(normalizeText(item?.status) || 'new')}</td>
</tr>`;
    }).join('');
    // Description block
    ...
    const buttonsHtml = isExecutor && openUrl
        ...
        : confirmUrl
            ? emailButtonsRow([
                ...
                emailButtonOutline(rescheduleUrl || '#', 'Перенести', '#D97706'),
            ])
            : '';
    // Heading
...
export const sendMaintenancePlanEmail = async (params) => {
  ...
  <td style="padding:10px 12px;border-bottom:1px solid #E2E8F0;">${systemBadges || '<span style="color:#94A3B8;">—</span>'}</td>
    ...
    const regulationsHtml = Array.from(activeSystems).map((key) => {
        const regulation = maintenanceSystemRegulations[key];
        const color = systemBadgeColors[key] || '#64748B';
        return `<tr>
...
export const maintenanceSystemRegulations = {
    aps: {
        ...
        work: 'Проверка шлейфов, извещателей, ПКП. Визуальный осмотр, тест сработки.',
    },
    soue: {
