...
export const installationStageSet: Set<string> = new Set(installationStageValues);

export const installationStageLabelMap: Record<string, string> = {
    new: 'Новая',
...
export const installationStageTransitionMap = new Map<string, Set<string>>([
...
export const installationDeadlineStages = ['procurement', 'in_progress', 'pnr', 'acceptance'] as const;
export const installationDeadlineStageSet = new Set<string>(installationDeadlineStages);

export const installationCancelReasons = ['client_refused', 'duplicate', 'no_access', 'moved_to_other', 'other'] as const;
export const installationCancelReasonSet = new Set<string>(installationCancelReasons);

export const installationCancelReasonLabels: Record<string, string> = {
...
export const procurementItemStatuses = ['needed', 'ordered', 'received'] as const;
export const procurementItemStatusSet = new Set<string>(procurementItemStatuses);
...
export const installationStageToNotificationEventMap: Record<string, string> = {
...
export const resolveInstallationNotificationEventType = (stage: string, fallback = 'installation_stage_changed'): string => {
...
export const normalizeInstallationStage = (value: any, fieldName = 'stage'): string => {
...
export const assertInstallationStageTransition = (currentStage: string, nextStage: string): void => {
...
export const normalizeProcurementItemStatus = (value: any, fieldName = 'status'): string => {
    const normalized = normalizeText(value).toLowerCase();
    if (!normalized) return 'needed';
...
export const installationStageDeadlinesUpdateSchema = z.object({
    ...
    reason: z.string().trim().min(2).max(2000).optional().default('Обновлено из интерфейса'),
});
...
export const chatMessagePinSchema = z.object({
    ...
    pinned: value.pinned ?? value.isPinned ?? false,
}));
...
export const buildProcurementSummary = (items: Array<{ status: string }>): ProcurementSummary => {
...
export const mapProcurementItemRow = (row: any) => ({
...
export const mapStageDeadlineRow = (row: any) => ({
