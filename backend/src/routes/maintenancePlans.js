export const registerMockMaintenancePlanRoutes = (params) => {
    const { app, requireAuth, ApiError, randomUUID, normalizeText, maintenancePlanManageRoles, projectAdminRoles, getLocalMockBodyObject, localMockUsers, localMockNotifications, localMockDirections, localMockMaintenanceItems, generateIcsEvent, wrapIcsCalendar, sendIcsResponse, publicLimiter, } = params;
...
export const registerMaintenancePlanRoutes = (params) => {
    const { app, requireAuth, asyncHandler, ensureMaintenanceSchema, ensureServiceRequestSchema, ensureNotificationSchema, ensureProjectSchema, ApiError, dbQuery, withTx, normalizeText, isUuidValue, isEmailValue, parseUuidPath, maintenancePlanManageRoles, projectAdminRoles, mapMaintenancePlanRow, mapGenerationRunRow, sendMaintenancePlanEmail, getAdminManagerIds, pushInAppNotification, pushTelegramNotification, logger, serializeError, randomUUID, generateIcsEvent, wrapIcsCalendar, sendIcsResponse, publicLimiter, parseDateOnlyValue, parsePositiveInt, dispatchServiceRequestCreationNotifications, toCsvCell, } = params;
    app.get('/maintenance-plans/export', requireAuth, asyncHandler(async (_request, response) => {
    ...
    app.get('/maintenance-plans/export-xlsx', requireAuth, asyncHandler(async (_request, response) => {
        await ensureMaintenanceSchema();
    ...
    app.post('/maintenance-plans/generate', requireAuth, asyncHandler(async (request, response) => {
        ...
        const planQuery = planIdFilter
            ? await dbQuery(`select * from maintenance_plans
               ...
               and valid_from <= $2::date and (valid_to is null or valid_to >= $3::date)`, [planIdFilter, periodEnd, periodStart])
            : await dbQuery(`select * from maintenance_plans
        ...
        const allPlannedObjectIds = new Set();
        const missedObjects = [];
        const warnings = [];
                ...
                if (mode === 'execute') {
                    try {
                        ...
                        if (plan.notify_executor !== false && executorIds.length > 0) {
                            try {
                                const execResult = await dbQuery(`select lower(trim(email)) as email, coalesce(nullif(trim(full_name),''), split_part(email,'@',1)) as full_name
                                     from app_users where id = any($1::text[]) and status = 'active' and coalesce(trim(email),'') <> ''`, [executorIds]);
    ...
    app.get('/maintenance-plans', requireAuth, asyncHandler(async (request, response) => {
        ...
        if (q.status && typeof q.status === 'string' && VALID_PLAN_STATUSES.has(q.status)) {
            paramIdx++;
            conditions.push(`mp.status = $${paramIdx}`);
            params.push(q.status);
        }
        if (q.directionId && typeof q.directionId === 'string') {
            paramIdx++;
            conditions.push(`mp.direction_id = $${paramIdx}`);
            params.push(normalizeText(q.directionId));
        }
        if (q.executorId && typeof q.executorId === 'string') {
            paramIdx++;
            conditions.push(`$${paramIdx} = any(mp.default_executor_ids)`);
            params.push(normalizeText(q.executorId));
        }
        if (q.tenantId && typeof q.tenantId === 'string') {
            paramIdx++;
            conditions.push(`mp.tenant_id = $${paramIdx}`);
            params.push(normalizeText(q.tenantId));
        }
        if (q.search && typeof q.search === 'string' && q.search.trim()) {
            paramIdx++;
            conditions.push(`(mp.name ilike $${paramIdx} OR mp.system_type ilike $${paramIdx})`);
