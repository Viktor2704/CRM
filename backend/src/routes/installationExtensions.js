...
export const registerInstallationExtensionRoutes = (params) => {
    const { app, requireAuth, asyncHandler, ensureInstallationExtendedSchema, ensureProjectSchema, ApiError, dbQuery, withTx, normalizeText, isUuidValue, parseUuidPath, randomUUID: genUUID, logger, serializeError, mapProjectRow, mapProjectEventRow, mapProjectChatMessageRow, insertProjectEventTx, sendProjectNotificationBestEffort, pushProjectInAppNotification, 
    // roles
    projectCreateRoles, projectUpdateRoles, projectDeleteRoles, projectAdminRoles, projectOpsRoles, projectClientRoles, projectFilesEditRoles, projectChatWriteRoles, installationProcurementRoles, installationStageEditRoles, installationDeadlineEditRoles, 
    // installation helpers
    installationStageSet, installationStageLabelMap, installationStageTransitionMap, installationDeadlineStageSet, installationCancelReasonSet, normalizeInstallationStage, assertInstallationStageTransition, normalizeProcurementItemStatus, resolveInstallationNotificationEventType, buildProcurementSummary, 
    // schemas
    installationStageUpdateSchema, installationStageDeadlinesUpdateSchema, procurementItemCreateSchema, procurementItemUpdateSchema, chatMessagePinSchema, 
    // mappers
    mapProcurementItemRow, mapStageDeadlineRow, appendProjectScopeCondition, projectSelectColumnsSql, } = params;
    ...
    const loadInstallation = async (installationId, queryFn = dbQuery) => {
        const result = await queryFn(`SELECT ${projectSelectColumnsSql},
             ...
             WHERE r.id = $1::uuid
               ...
               AND r.deleted_at IS NULL`, [installationId]);
    ...
    app.get('/installations/:installationId/extended', requireAuth, asyncHandler(async (request, response) => {
        ...
        // Stage deadlines
        const deadlinesResult = await dbQuery(`SELECT stage, due_date FROM installation_stage_deadlines WHERE installation_id = $1::uuid ORDER BY due_date`, [installationId]);
        const stageDeadlines = {};
        for (const dr of deadlinesResult.rows) {
            const mapped2 = mapStageDeadlineRow(dr);
            if (mapped2.stage)
                stageDeadlines[mapped2.stage] = mapped2.dueDate;
        }
        // Procurement summary
        const procResult = await dbQuery(`SELECT status FROM installation_procurement_items WHERE installation_id = $1::uuid`, [installationId]);
    ...
    app.patch('/installations/:installationId/installation-stage', requireAuth, asyncHandler(async (request, response) => {
        ...
        if (currentStage === 'procurement' && nextStage === 'in_progress') {
            ...
            if (!summary.allReceived && summary.total > 0) {
                if (!body.overrideReason || body.overrideReason.trim().length < 10) {
                    throw new ApiError(422, 'PROCUREMENT_NOT_COMPLETE', 'Не все позиции закупки получены. Укажите overrideReason (мин. 10 символов) для обхода.');
                }
            }
        }
        // ── Gating: → cancelled ──
        ...
        await withTx(async (tx) => {
            const updates = [`status = $2::text`, `updated_at = now()`];
            const values = [installationId, nextStage];
            if (nextStage === 'paused') {
            ...
            if (body.overrideReason)
                eventPayload.overrideReason = body.overrideReason;
            if (body.cancelReason)
                eventPayload.cancelReason = body.cancelReason;
            if (body.cancelComment)
                eventPayload.cancelComment = body.cancelComment;
            if (body.pauseReason)
                eventPayload.pauseReason = body.pauseReason;
            ...
            if (body.reason)
                systemMsg += `. Причина: ${body.reason}`;
            if (body.overrideReason)
                systemMsg += `. Override: ${body.overrideReason}`;
            if (body.pauseReason)
                systemMsg += `. Причина паузы: ${body.pauseReason}`;
            if (body.cancelComment)
                systemMsg += `. Комментарий: ${body.cancelComment}`;
            await tx.query(`INSERT INTO project_chat_messages (id, project_id, author_id, visibility, text, created_at)
                 VALUES ($1::uuid, $2::uuid, $3::uuid, 'client-visible', $4, now())`, [uuid(), installationId, actorId || null, systemMsg]);
        });
    ...
    app.get('/installations/:installationId/stage-deadlines', requireAuth, asyncHandler(async (request, response) => {
        await ensureSchemas();
        const installationId = parseUuidPath(request.params.installationId);
        const result = await dbQuery(`SELECT stage, due_date FROM installation_stage_deadlines WHERE installation_id = $1::uuid ORDER BY due_date`, [installationId]);
        response.status(200).json({
    ...
    app.put('/installations/:installationId/stage-deadlines', requireAuth, asyncHandler(async (request, response) => {
        ...
        const oldResult = await dbQuery(`SELECT stage, due_date FROM installation_stage_deadlines WHERE installation_id = $1::uuid`, [installationId]);
        const oldMap = {};
        ...
        await withTx(async (tx) => {
            for (const d of body.deadlines) {
                await tx.query(`INSERT INTO installation_stage_deadlines (installation_id, stage, due_date, updated_at)
                     VALUES ($1::uuid, $2, $3::date, now())
                     ON CONFLICT (installation_id, stage)
                     DO UPDATE SET due_date = $3::date, updated_at = now()`, [installationId, d.stage, d.dueDate]);
            }
            // Event for each changed deadline
            for (const d of body.deadlines) {
                const oldDate = oldMap[d.stage] || null;
                if (oldDate === d.dueDate)
                    continue;
                await insertProjectEventTx({
                    client: tx,
                    projectId: installationId,
                ...
                await tx.query(`INSERT INTO project_chat_messages (id, project_id, author_id, visibility, text, created_at)
                     VALUES ($1::uuid, $2::uuid, $3::uuid, 'client-visible', $4, now())`, [uuid(), installationId, actorId || null, msg]);
            }
    ...
    app.get('/installations/:installationId/procurement', requireAuth, asyncHandler(async (request, response) => {
        await ensureSchemas();
        const installationId = parseUuidPath(request.params.installationId);
        const result = await dbQuery(`SELECT * FROM installation_procurement_items
             WHERE installation_id = $1::uuid
             ORDER BY created_at ASC`, [installationId]);
    ...
    app.post('/installations/:installationId/procurement', requireAuth, asyncHandler(async (request, response) => {
        ...
        const id = uuid();
        await dbQuery(`INSERT INTO installation_procurement_items
    ...
    app.patch('/installations/:installationId/procurement/:itemId', requireAuth, asyncHandler(async (request, response) => {
        ...
        // Check item exists
        const existing = await dbQuery(`SELECT * FROM installation_procurement_items WHERE id = $1::uuid AND installation_id = $2::uuid`, [itemId, installationId]);
        ...
        if (body.responsibleUserId !== undefined) {
            vals.push(body.responsibleUserId || null);
            sets.push(`responsible_user_id = $${idx++}`);
        }
        if (body.files !== undefined) {
            vals.push(JSON.stringify(body.files));
            sets.push(`files = $${idx++}::jsonb`);
        }
        ...
        if (body.status === 'received') {
            const allResult = await dbQuery(`SELECT status FROM installation_procurement_items WHERE installation_id = $1::uuid`, [installationId]);
            const summary = buildProcurementSummary(allResult.rows);
            if (summary.allReceived) {
                // System chat message
                await dbQuery(`INSERT INTO project_chat_messages (id, project_id, author_id, visibility, text, created_at)
                     VALUES ($1::uuid, $2::uuid, $3::uuid, 'client-visible', 'Закупка: все позиции получены', now())`, [uuid(), installationId, actorId || null]);
                try {
    ...
    app.delete('/installations/:installationId/procurement/:itemId', requireAuth, asyncHandler(async (request, response) => {
        ...
        const installationId = parseUuidPath(request.params.installationId);
        const itemId = parseUuidPath(request.params.itemId);
        const existing = await dbQuery(`SELECT name FROM installation_procurement_items WHERE id = $1::uuid AND installation_id = $2::uuid`, [itemId, installationId]);
        if (existing.rows.length === 0) {
            throw new ApiError(404, 'NOT_FOUND', 'Позиция закупки не найдена');
        }
    ...
    app.patch('/installations/:installationId/chat/messages/:messageId/pin', requireAuth, asyncHandler(async (request, response) => {
        ...
        const result = await dbQuery(`UPDATE project_chat_messages
             SET is_pinned = $3, edited_at = now()
             WHERE id = $1::uuid AND project_id = $2::uuid AND deleted_at IS NULL
             RETURNING *`, [messageId, installationId, body.pinned]);
    ...
    app.get('/installations/:installationId/chat/pinned', requireAuth, asyncHandler(async (request, response) => {
        await ensureSchemas();
        const installationId = parseUuidPath(request.params.installationId);
        const result = await dbQuery(`SELECT * FROM project_chat_messages
             WHERE project_id = $1::uuid AND is_pinned = true AND deleted_at IS NULL
             ORDER BY created_at DESC`, [installationId]);
        response.status(200).json({
