...
export const canExportDirections = canUseInternalGlobalExport;
...
export const registerMockDirectionRoutes = (params) => {
    ...
    app.get('/directions/:directionId', requireAuth, (request, response) => {
        const actorRole = request.authUser?.role ?? '';
        if (!directionViewRoles.has(actorRole)) {
            throw new ApiError(403, 'FORBIDDEN', 'Not allowed');
        }
    ...
    app.patch('/directions/:directionId', requireAuth, (request, response) => {
        const actorRole = request.authUser?.role ?? '';
        if (!directionManageRoles.has(actorRole)) {
            throw new ApiError(403, 'FORBIDDEN', 'Not allowed');
        }
    ...
    app.delete('/directions/:directionId', requireAuth, (request, response) => {
        const actorRole = request.authUser?.role ?? '';
        if (!directionManageRoles.has(actorRole)) {
            throw new ApiError(403, 'FORBIDDEN', 'Not allowed');
        }
    ...
    app.get('/directions/export-xlsx', requireAuth, async (request, response) => {
        const actorRole = request.authUser?.role ?? '';
        if (!canExportDirections(actorRole, internalGlobalRoles)) {
            throw new ApiError(403, 'FORBIDDEN', 'Not allowed');
        }
        ...
        ], localMockDirections.map((direction) => ({
            id: String(direction.id ?? ''),
            name: String(direction.name ?? ''),
            address: String(direction.address ?? ''),
            tenantName: String(direction.tenantName ?? ''),
            description: String(direction.description ?? ''),
            createdAt: direction.createdAt ? String(direction.createdAt).slice(0, 10) : '',
        })));
    });
    app.post('/directions/:directionId/items/import-xlsx', requireAuth, async (request, response) => {
        const actorRole = request.authUser?.role ?? '';
        if (!directionManageRoles.has(actorRole)) {
            throw new ApiError(403, 'FORBIDDEN', 'Not allowed');
        }
        ...
        for (const xlsxRow of xlsxRows) {
            ...
            const existingIndex = localMockMaintenanceItems.findIndex((item) =>
                item.directionId === request.params.directionId
                && normalizeText(item.name).toLowerCase() === name.toLowerCase());
            if (existingIndex >= 0) {
                localMockMaintenanceItems[existingIndex] = {
                    ...localMockMaintenanceItems[existingIndex],
                    address: mapped.address || localMockMaintenanceItems[existingIndex].address,
                    legalEntity: mapped.legalEntity || localMockMaintenanceItems[existingIndex].legalEntity,
                    contractNumber: mapped.contractNumber || localMockMaintenanceItems[existingIndex].contractNumber,
                    updatedAt: new Date().toISOString(),
                };
                updated += 1;
            }
            else {
                localMockMaintenanceItems.push({
                    id: randomUUID(),
                    directionId: request.params.directionId,
                    positionNumber: String(localMockMaintenanceItems.filter((item) => item.directionId === request.params.directionId).length + 1),
                    name,
                    address: mapped.address,
                    legalEntity: mapped.legalEntity,
                    contractNumber: mapped.contractNumber,
                    tenantId: normalizeText(direction.tenantId) || null,
                    systems: {
                        aps: false,
                        soue: false,
                        aupt: false,
...
export const registerDirectionRoutes = (params) => {
    ...
    const tenantIdValue = normalizeText(body.tenantId);
    if (tenantIdValue) {
        const tenantCheck = await dbQuery('SELECT coalesce(type, \'tenant\') as type FROM tenants WHERE id = $1::uuid AND deleted_at IS NULL', [tenantIdValue]);
        if (tenantCheck.rows.length > 0 && tenantCheck.rows[0].type !== 'tenant') {
            throw new ApiError(422, 'VALIDATION_ERROR', 'Only tenant-type tenants can be assigned to directions');
        }
    }
    ...
    app.patch('/directions/:directionId', requireAuth, asyncHandler(async (request, response) => {
    await ensureDirectionSchema();
    const actorRole = request.authUser?.role ?? '';
    if (!directionManageRoles.has(actorRole)) {
    ...
    if (body.tenantId !== undefined) {
        ...
        sets.push(`tenant_id = ${push(tenantIdValue)}`);
    }
    if (body.description !== undefined) {
    ...
    app.delete('/directions/:directionId', requireAuth, asyncHandler(async (request, response) => {
    await ensureDirectionSchema();
    const actorRole = request.authUser?.role ?? '';
    if (!directionManageRoles.has(actorRole)) {
    ...
    const result = await dbQuery(`select d.*, t.brand_name as tenant_name
       from directions d
       left join tenants t on t.id::text = d.tenant_id::text
    ...
    app.get('/directions/:directionId', requireAuth, asyncHandler(async (request, response) => {
    await ensureDirectionSchema();
    const actorRole = request.authUser?.role ?? '';
    if (!directionViewRoles.has(actorRole)) {
        throw new ApiError(403, 'FORBIDDEN', 'Not allowed');
    ...
    for (const row of rows) {
        ...
        if (existing.rows.length > 0) {
            ...
            await dbQuery(`insert into directions (name, address, tenant_id, created_by_id) values ($1, $2, $3::uuid, $4::uuid)`,
                [name, address, tenantId, normalizeText(request.authUser?.id) || null]);
            created++;
        }
    }
    response.status(200).json({ summary: { created, updated } });
    }));
    app.post('/directions/:directionId/items/import-xlsx', requireAuth, asyncHandler(async (request, response) => {
    await ensureDirectionSchema();
    await ensureMaintenanceItemSchema();
    const actorRole = request.authUser?.role ?? '';
    if (!directionManageRoles.has(actorRole)) throw new ApiError(403, 'FORBIDDEN', 'Not allowed');
    const directionId = parseUuidPath(request.params.directionId);
    const dirCheck = await dbQuery(`SELECT id FROM directions WHERE id=$1::uuid AND deleted_at IS NULL LIMIT 1`, [directionId]);
    if (dirCheck.rows.length === 0) throw new ApiError(404, 'NOT_FOUND', 'Direction not found');
    const contentType = String(request.headers['content-type'] || '');
    const chunks: Buffer[] = [];
    ...
    const { parseXlsxBuffer, mapDirectionItemXlsxRow } = await import('../helpers/xlsxImportHelpers.js');
    const xlsxRows = await parseXlsxBuffer(xlsxBuffer);
    if (xlsxRows.length === 0) throw new ApiError(422, 'VALIDATION_ERROR', 'Файл пустой');
    let created = 0;
    let updated = 0;
    for (const xlsxRow of xlsxRows) {
        const mapped = mapDirectionItemXlsxRow(xlsxRow);
        ...
        if (existing.rows.length > 0) {
            await dbQuery(`UPDATE maintenance_items SET address=coalesce(nullif($2,''),address), legal_entity=coalesce(nullif($3,''),legal_entity), contract_number=coalesce(nullif($4,''),contract_number), updated_at=now() WHERE id=$1::uuid`,
                [existing.rows[0].id, mapped.address, mapped.legalEntity, mapped.contractNumber]);
            updated++;
        } else {
            await dbQuery(`INSERT INTO maintenance_items(direction_id, position_number, name, address, legal_entity, contract_number) VALUES($1::uuid,$2,$3,$4,$5,$6)`,
                [directionId, mapped.positionNumber, name, mapped.address, mapped.legalEntity, mapped.contractNumber]);
            created++;
        }
    }
    response.status(200).json({ summary: { created, updated } });
