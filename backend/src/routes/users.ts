export const registerMockUsersRoutes = (params) => {
    ...
    app.patch('/users/me', requireAuth, requireCsrfToken, (request, response) => {
        const user = localMockUsers.find((candidate) => candidate.id === request.authUser.id);
        if (!user) {
            response.status(404).json({ code: 'USER_NOT_FOUND', message: 'User not found' });
            return;
    ...
    app.get('/users/export', requireAuth, requireUserPageAccess, (request, response) => {
        const csvRows = localMockUsers.map((user) => [
            user.id,
            user.fullName,
            user.email,
            user.role,
    ...
    app.get('/users/export-xlsx', requireAuth, requireUserPageAccess, async (_request, response) => {
        ...
        ], localMockUsers.map((user) => ({
            id: String(user.id ?? ''),
            fullName: String(user.fullName ?? ''),
            email: String(user.email ?? ''),
            role: String(user.role ?? ''),
            status: String(user.status ?? ''),
            createdAt: user.createdAt ? String(user.createdAt).slice(0, 10) : '',
        })));
    });
...
export const registerUsersRoutes = (params) => {
    ...
    app.get('/users/export', requireAuth, requireUserPageAccess, asyncHandler(async (_request, response) => {
    ...
    app.get('/users/export-xlsx', requireAuth, requireUserPageAccess, asyncHandler(async (_request, response) => {
        const result = await dbQuery(`SELECT id, full_name, email, role, status, created_at FROM app_users ORDER BY created_at DESC`);
        const { sendXlsxResponse } = await import('../helpers/xlsxHelpers.js');
        await sendXlsxResponse(response, 'users', [
            { header: 'ID', key: 'id', width: 38 },
    ...
    app.get('/audit/users/stream', asyncHandler(async (request, response) => {
        request.authUser = await resolveStreamAuthUser(request, { requireAdminLike: true });
        response.status(200);
