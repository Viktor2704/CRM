export const registerMockCheckinRoutes = (params) => {
    const { app, requireAuth, ApiError, randomUUID, normalizeText, maintenanceItemViewRoles, publicLimiter, localMockUsers, localMockNotifications, localMockCheckins, localMockMaintenanceItems, } = params;
...
export const registerCheckinRoutes = (params) => {
    const { app, requireAuth, asyncHandler, ensureMaintenanceItemSchema, ensureNotificationSchema, ApiError, dbQuery, normalizeText, parseUuidPath, publicLimiter, getAdminManagerIds, pushInAppNotification, logger, serializeError, } = params;
